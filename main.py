...
class TextBookParser:
    """
    解析TXT文本文件，生成MultiLevelBook结构，使用新的正则表达式匹配卷和章节。
    """
    # 新的正则表达式，用于匹配卷和章节
    TITLE_PATTERN = r"^[\s\u3000]*[第卷][0-9一二三四五六七八九十零〇百千两]+[章回部节集卷篇辑].*"
    ...
    def read(file_path: str) -> MultiLevelBook:
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            current_chapter = '默认章节'  # 初始化为默认章节
            multi_level_book.add_volume('默认卷')  # 添加默认卷
            multi_level_book.add_chapter_to_last_volume(current_chapter) # 添加默认章节
            current_content = []
            ...
            for line in file:
                ...
                # 如果该行匹配卷或章节的标题
                if re.match(TextBookParser.TITLE_PATTERN, line):
                    print(f"匹配到标题: {line}")  # 调试输出

                    if current_chapter and current_content:
                        multi_level_book.add_content_to_last_chapter('\n'.join(current_content))
                        current_content = []

                    # 如果是卷标题（例如，第1卷）
                    if "卷" in line:
                        current_volume = line  # 记录卷标题
                        multi_level_book.add_volume(current_volume)  # 添加新卷
                        current_chapter = None  # 重置当前章节
                    # 如果是章节标题（例如，第1章，第2回）
                    elif "章" in line or "回" in line or "节" in line:
                        current_chapter = line
                        multi_level_book.add_chapter_to_last_volume(current_chapter)  # 添加新章节

                # 其他内容，添加到当前章节内容列表
                elif line:
                    current_content.append(line)
                    print(f"添加到章节内容: {line}")
        ...
        for volume_index, volume in enumerate(book_structure.volumes, start=1):
            ...
            for chapter_index, chapter in enumerate(volume['chapters'], start=1):
                ...
                with open(chap_file_path, 'r', encoding="utf8") as f:
                    fcontent = f.read()
                    print(f"读取文件: {chap_file_path}")  # 调试输出
                    #print(f"文件内容: {fcontent}")  # 调试输出
                # 创建EpubHtml对象代表章节
...
def convert_all_txt_in_directory(directory_path: str):
    ...
    for txt_file in txt_files:
        ...
        epub_path = os.path.join(directory_path, f"{os.path.splitext(txt_file)[0]}.epub")
        cover_image = os.path.join(directory_path, f"{os.path.splitext(txt_file)[0]}.jpg")  # 默认封面
        ...
        print(f"开始转换文件: {txt_file}")
        converter.convert()
        print(f"文件转换完成: {txt_file}")
